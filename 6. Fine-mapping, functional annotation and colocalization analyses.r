...
meta_file_path <- './EAS_EUR_meta_fixed.txt'
meta_sample_size <- 539860
chunk_size <- 500000
soft_dir <- './software/polyfun-master'
res_dir <- './susie/PolyFun'
meta <- fread(meta_file_path)
for (i in 1:nrow(list)) {
  system(paste0('plink --noweb --bfile ./chr_all_1kgv3_2015_',people,' --snp ',list$rsid[i],' --window 500 --make-bed --out ',people,'/',type,'/',list$rsid[i],'/',list$rsid[i]))
...
data<-gwas[,c('variant_id','lead_snp','rsid','R2')]
...
gwas_files <- list.files(path = "./", pattern = "*.txt", full.names = TRUE)
...
model <- glm(MPHOSPH6~C,data=data)
...
## ==============================================================================
## smoking behavior, LC GWAS, brain tissue eQTL, lung tissue eQTL hyprcoloc
## Give an example
e <- fread('./GTEx_v10/Lung.v10.b38.eqt.forColoc.csv.gz')
gwas <- fread('./LC_GWAS_lead_SNP/rs6011779.txt')
smoke <- fread('./smoking_GWAS_hg38/smoking_GWAS_for_hyprcoloc/SI_gwas_hg38_for_hyprcoloc.txt')

setkey(e, SNP); setkey(gwas, SNP); setkey(smoke, SNP)
m <- gwas[e, .(SNP, beta_gwas, se_gwas), nomatch = 0L][
     smoke, on = "SNP", nomatch = 0L][
     e,     on = "SNP", nomatch = 0L]

betas <- as.matrix(m[, .(beta_gwas, beta_smoke, beta_eqtl)])
ses   <- as.matrix(m[, .(se_gwas,   se_smoke,   se_eqtl)])
colnames(betas) <- colnames(ses) <- c("LC","SMK","eQTL")
rownames(betas) <- m$SNP

smk_is_binary <- TRUE
bin_flags <- c(1, as.integer(smk_is_binary), 0)

## run hyprcoloc
res  <- hyprcoloc(effect.est = betas,
                  effect.se  = ses,
                  binary.outcomes = bin_flags,
