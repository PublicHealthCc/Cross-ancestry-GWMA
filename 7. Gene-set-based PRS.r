...
## Give an example level 1 gene GO 
R
library(data.table)
...
for (j in prs_list) {
  ...
  for (i in c(242:261)){
    ...
    formula <- as.formula(paste0(names(dat)[i], "~ ",j))
  ...
  write.csv(res_out1,file =  paste0('logit_race_age_impute_NSCLC_',j,'Crude_res.csv'),row.names = F)
}  
...
for (i in type) {
  ...
  for (j in prs_list) {
    ...
    dat1$race_use <- as.factor(dat1$race_use)
    formula <- as.formula(paste0('Surv(',i,'.time,',i,') ~  ',j))
  ...
  write.csv(res_out1,file = paste0('race_age_impute_NSCLC_Crude_',i,'.csv'),row.names = F)
}
...
for (i in prs_list) {
  ...
  write.csv(res_signature,file = paste0('race_age_impute_NSCLC_Crude_',i,'.csv'),row.names = F)
}
...
write.csv(res_signature,file = paste0('~/TMB/race_age_impute_NSCLC_Crude_TMB.csv'),row.names = F)
